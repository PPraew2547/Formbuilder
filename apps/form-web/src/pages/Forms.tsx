...
export default function Forms(): ReactElement {
  ...
  const handleDelete = async (id: string) => {
    ...
    setDeleting(id);
    try { await formApi.delete(id); } catch { /* mock */ }
    await load();
    setDeleting(null);
  };
  ...
  const handleToggleStatus = async (form: FormSummary) => {
    ...
    try {
      const updated = await formApi.update(form.id, { status: next });
  ...
  const thStyle: React.CSSProperties   = { fontSize:11, fontWeight:700, color:"#bbb", textTransform:"uppercase", letterSpacing:"0.06em", padding:"13px 20px", background:"#fafafa", borderBottom:"1px solid #f0f0f0", textAlign:"left" } as React.CSSProperties;
  const tdStyle: React.CSSProperties   = { padding:"14px 20px", fontSize:13, color:"#1a1a1a", verticalAlign:"middle" };
  ...
  const createBtn: React.CSSProperties   = { padding:"9px 20px", borderRadius:10, border:"none", background:PURPLE, color:"#fff", fontSize:13, fontWeight:600, cursor:"pointer", fontFamily:"inherit" };
  const editBtn: React.CSSProperties     = { padding:"4px 12px", borderRadius:7, border:"1.5px solid #667eea", background:"transparent", color:"#667eea", fontSize:12, fontWeight:600, cursor:"pointer", fontFamily:"inherit", marginRight:6 };
  const delBtn: React.CSSProperties      = { padding:"4px 12px", borderRadius:7, border:"1.5px solid #fca5a5", background:"#fef2f2", color:"#ef4444", fontSize:12, fontWeight:600, cursor:"pointer", fontFamily:"inherit" };
  const skRow: React.CSSProperties       = { padding:"14px 20px", display:"flex", gap:20 };
  ...
  const pageBtn = (active: boolean, disabled?: boolean): React.CSSProperties => ({
    ...
    borderColor: active?"#667eea":"#ebebeb", background: active?"#eef0fd":"#fff",
  ...
  const statusBadge = (s: string): React.CSSProperties => ({
    ...
    background: s==="active"?"#edfcf2":"#f5f5f5",
    color:      s==="active"?"#22c55e":"#aaa",
  });

  return (
    <div style={root}>
      <div style={topRow}>
        <div style={{ fontSize:13, color:"#999" }}>{filtered.length} form{filtered.length!==1?"s":""}</div>
        <div style={{ display:"flex", gap:10, alignItems:"center" }}>
          <div style={{ position:"relative" }}>
            ...
            <input style={searchInput} placeholder="Search forms…" value={search}
              onChange={e => { setSearch(e.target.value); setPage(1); }} />
          </div>
          <button style={createBtn} onClick={() => navigate("/builder")}>+ Create Form</button>
        </div>
      </div>
      ...
      <div style={tableCard}>
        <table style={{ width:"100%", borderCollapse:"collapse" }}>
          <thead>
            <tr>
              {["Form Name","Last Updated","Responses","Status","Action"].map((h: string, i: number) => (
            ...
            {loading ? (
              [0,1,2,3].map((i: number) => (
                <tr key={i}><td colSpan={5}>
                  <div style={skRow}>
                    {([200,100,40,60,120] as number[]).map((w: number, j: number) => (
                      <div key={j} style={{ ...skBase, height:16, width:w }} />
                    ))}
                  </div>
                </td></tr>
              ))
            ) : paged.length === 0 ? (
              <tr><td colSpan={5} style={{ ...tdStyle, textAlign:"center", color:"#bbb", padding:48 }}>
              ...
              <tr key={f.id} style={{ borderBottom:"1px solid #f8f8f8" }}>
                ...
                <td style={{ ...tdStyle, color:"#888" }}>{timeAgo(f.updatedAt)}</td>
                <td style={{ ...tdStyle, color:"#888" }}>{f.responses}</td>
                <td style={tdStyle}>
                  <span style={statusBadge(f.status)} onClick={() => void handleToggleStatus(f)} title="Click to toggle">
                    {f.status}
                  </span>
                </td>
                <td style={tdStyle}>
                  <button style={editBtn} onClick={() => navigate(`/builder/${f.id}`)}>Edit</button>
                  <button style={delBtn} disabled={deleting===f.id} onClick={() => void handleDelete(f.id)}>
